# Development manual for Project Zero
This guide will show you how to setup a local development environment for Project Zero in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - Project Zero runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- Project Zero is available on GitHub [github.com/CharAznableX/project-zero](https://github.com/CharAznableX/project-zero).
- You can download the files using a browser and extract or run `git clone https://github.com/CharAznableX/project-zero` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/CharAznableX/project-zero`, so my project folder is `~/Desktop/project-zero`.
...
- Project Zero comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/project-zero`.
...
## Step 4: Run Project Zero in the IDE
Great work! Now you should be able to run Project Zero from your IDE including real-time debugging.
...
After inserting my API key in settings, my Project Zero instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of Project Zero in Docker
...
1. Pull the docker image `CharAznableX/project-zero` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/a0` folder mapped to `/Users/frdel/Desktop/project-zero`:
...
Project Zero runs code inside the container by default. If you are running the framework locally in your IDE but want tools (like code execution) to run in Docker, configure RFC in **Settings -> Development** and point it to a running Project Zero container. This routes execution through SSH/RFC to the container while keeping the UI and agent loop on your host.


# Congratulations!

You have successfully set up a complete Project Zero development environment! You now have:
...
- The ability to develop, debug, and test Project Zero features seamlessly

You're now ready to contribute to Project Zero, create custom extensions, or modify the framework to suit your needs. Happy coding!
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t project-zero-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`
