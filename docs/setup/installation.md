...
Docker Desktop provides the runtime environment for Project Zero, ensuring consistent behavior and security across platforms. The entire framework runs within a Docker container, providing isolation and easy deployment.
...
### Continue to [Step 2: Run Project Zero](#step-2-run-project-zero)
...
## Step 2: Run Project Zero

### 2.1. Pull the Project Zero Docker Image

**Using Docker Desktop GUI:**

- Search for `CharAznableX/project-zero` in Docker Desktop
...
Choose or create a folder on your computer where Project Zero will save its data. 
...
- **Windows:** `C:\project-zero-data`
- **macOS/Linux:** `/home/user/project-zero-data`
...
> [!TIP]
> Choose a location that's easy to access and backup. All your Project Zero data will be directly accessible in this directory.
...
- Click the `Run` button next to the `CharAznableX/project-zero` image
...
Open `http://localhost:<PORT>` in your browser. The Web UI will open - Project Zero is ready for configuration!
...
> [!NOTE]
> After starting the container, you'll find all Project Zero files in your chosen directory. You can access and edit these files directly on your machine, and the changes will be immediately reflected in the running container.

**Running A0 using Terminal?**

```bash
docker run -p 0:80 -v /path/to/your/work_dir:/a0/usr CharAznableX/project-zero
```
...
Project Zero provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings" button with a gear icon in the sidebar.
...
> [!NOTE]
> Project Zero uses a local embedding model by default (runs on CPU), but you can switch to OpenAI embeddings like `text-embedding-3-small` or `text-embedding-3-large` if preferred.
...
> [!NOTE]
> **OpenAI API vs Plus subscription:** A ChatGPT Plus subscription does not include API credits. You must provide a separate API key for OpenAI usage in Project Zero.
...
> [!CAUTION]
> **GitHub Copilot Provider:** When using the GitHub Copilot provider, after selecting the model and entering your first prompt, the OAuth login procedure will begin. You'll find the authentication code and link in the output logs. Complete the authentication process by following the provided link and entering the code, then you may continue using Project Zero.
...
The Settings page is the control center for selecting the Large Language Models (LLMs) that power Project Zero. You can choose different LLMs for different roles:
...
### Configuring Ollama in Project Zero
...
> [!NOTE]
> If Project Zero runs in Docker and Ollama runs on the host, ensure port **11434** is reachable from the container. If both services are in the same Docker network, you can use `http://<container_name>:11434` instead of `host.docker.internal`.
...
## How to Update Project Zero

> [!NOTE]
> Since v0.9, Project Zero includes a Backup & Restore workflow in the Settings UI. This is the **safest** way to upgrade Docker instances.

### Recommended Update Process (Docker)

1. **Keep the old container running** and note its port.
2. **Pull the new image** (`CharAznableX/project-zero:latest`).
...
## Using Project Zero on Your Mobile Device

Project Zero can be accessed from mobile devices and other computers using the built-in **Tunnel feature**.

### Recommended: Using Tunnel (Remote Access)

The Tunnel feature allows secure access to your Project Zero instance from anywhere:
...
5. Share this URL to access Project Zero from any device
...
For developers or users who need to run Project Zero directly on their system, see the [In-Depth Guide for Full Binaries Installation](dev-setup.md).

---

## Advanced: Automated Configuration via Environment Variables

Project Zero settings can be automatically configured using environment variables with the `A0_SET_` prefix in your `.env` file. This enables automated deployments without manual configuration.
...
After following the instructions for your specific operating system, you should have Project Zero successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents.
...
If you encounter any issues during the installation process, please consult the [Troubleshooting section](../guides/troubleshooting.md) of this documentation or refer to the Project Zero [Skool](https://www.skool.com/project-zero) or [Discord](https://discord.gg/B8KZKNsPpj) community for assistance.
