# Quick Start
This guide provides a quick introduction to using Project Zero. We'll cover the essential installation steps and running your first Skill.
...
### Step 2: Pull the Project Zero Image

Using Docker Desktop GUI, search for `CharAznableX/project-zero` and click Pull, or use the terminal:
...
**Using Docker Desktop:** Go to Images tab, click Run next to `CharAznableX/project-zero`, open Optional settings, map a host port to container port `80` (use `0` for automatic assignment), then click Run.

**Using Terminal:**

```bash
docker run -p 0:80 CharAznableX/project-zero
```
...
![Project Zero Web UI](res/setup/6-docker-a0-running-new.png)
...
> [!NOTE]
> Project Zero supports any LLM provider, including local models via Ollama. For detailed provider configuration and local model setup, see the [Installation Guide](setup/installation.md#choosing-your-llms).

### Step 5: Start Your First Chat

Once configured, you'll see the Project Zero dashboard with access to:
...
Click **New Chat** to start creating with Project Zero!

![Project Zero Dashboard](res/quickstart/ui_newchat1.png)
...
## Example Interaction
Let's ask Project Zero to use one of the built-in skills. Here's how:

1. Type "Activate your brainstorming skill" in the chat input field and press Enter or click the send button.
2. Project Zero will process your request. You'll see its thoughts and tool calls in the UI.
...
## Next Steps
Now that you've run a simple task, you can experiment with more complex requests. Try asking Project Zero to:
