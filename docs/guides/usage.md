# Usage Guide
This guide explores usage and configuration scenarios for Project Zero. You can consider this as a reference post-installation guide.

![Utility Message with Solutions](../res/usage/first-task.png)

## Basic Operations
Project Zero provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the Project Zero container without using the terminal:
...
### Action Buttons
Located beneath the chat input box, Project Zero provides a set of action buttons for enhanced control and visibility:
...
### File Browser: Manage files in the Project Zero environment
...
### File Attachments
Project Zero supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
Project Zero's power comes from its ability to use [tools](../developer/architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** Project Zero includes default tools like knowledge (powered by SearXNG), code execution, and communication. Understand the capabilities of these tools and how to invoke them.
...
Project Zero instances can communicate with each other using the A2A protocol. This enables:
...
To enable A2A connectivity, go to **Settings → MCP/A2A → A0 A2A Server** and toggle the server on. You'll receive a connection URL that other Project Zero instances can use to communicate with your agent.

See [A2A Setup](a2a-setup.md) for detailed configuration and use cases.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want Project Zero to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the Project Zero Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of Project Zero to solve complex problems.

## Multi-Agent Cooperation
One of Project Zero's unique features is multi-agent cooperation.
...
* **Hierarchy:** Project Zero uses a [hierarchical structure](../developer/architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](../res/usage/multi-agent.png)

## Projects
Projects are isolated workspaces that provide dedicated context, instructions, memory, and secrets for specific tasks or clients. They are one of Project Zero's most powerful organizational features, preventing context bleed and enabling focused, specialized agent behavior.
...
3. Project Zero clones the repository and sets up the project structure
...
> [!NOTE]
> If the cloned repository already contains a `.a0proj/` configuration folder, Project Zero merges the existing configuration with your specified preferences.
...
## Tasks & Scheduling
Tasks enable Project Zero to run automated or scheduled work in isolated contexts. They're perfect for recurring workflows, batch processing, or time-delayed operations that don't require immediate attention.

### What Are Tasks?

Tasks are autonomous work units that Project Zero executes in dedicated or shared chat contexts. Each task includes:
...
Project Zero supports three types of tasks:
...
Project Zero includes a secure tunneling feature that allows you to expose your local instance to the internet. This makes it possible to access your Project Zero instance from anywhere or share it with others without complex network configuration.

### How Tunneling Works

Project Zero uses the [Flaredantic](https://pypi.org/project/flaredantic/) library to create secure tunnels. These tunnels:
...
6. The tunnel URL remains active until you stop the tunnel or close Project Zero

### Security Considerations

When sharing your Project Zero instance via a tunnel:

- Anyone with the URL can access your Project Zero instance
- No additional authentication is added beyond what your Project Zero instance already has
- **Always set up authentication before creating a tunnel** (see below)
- The tunnel exposes only your Project Zero instance, not your entire system

> [!IMPORTANT]
> When attempting to create a tunnel without authentication configured, Project Zero will display a security warning.

### Adding Authentication for Tunnels

To secure your tunneled Project Zero instance, configure authentication in Settings:

1. Open **Settings** in the Project Zero UI
...
This will require users to enter these credentials when accessing your tunneled Project Zero instance.
...
3. Restart Project Zero
4. Check the console logs for any error messages

> [!TIP]
> Combine tunneling with authentication for secure remote access to your Project Zero instance from any device, including mobile phones and tablets.

## Voice Interface
Project Zero provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
### File Browser
Project Zero provides a powerful file browser interface for managing your workspace:
...
> [!NOTE]
> The file browser lets you navigate the Project Zero filesystem. For file-based work, keep your working files in `/a0/usr` (or inside a Project workspace).
>
- **File Operations**:
...
> [!TIP]
> The File Browser integrates seamlessly with Project Zero's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.

## Memory Management
Project Zero includes a sophisticated memory management system that stores and retrieves information from conversations, knowledge sources, and learning experiences. The Memory Dashboard provides a powerful interface to view, search, filter, edit, and delete memory entries stored in the vector database.
...
## Backup & Restore
Project Zero provides a comprehensive backup and restore system to protect your data and configurations. This feature helps you safeguard your work and migrate Project Zero setups between different systems.
...
#### What Gets Backed Up
By default, Project Zero backs up your most important data:
...
The restore process allows you to recover your Project Zero setup from a previous backup:
...
* **Before System Updates**: Backup before updating Project Zero or system components
...
* **External Storage**: Keep backup files in a safe location outside Project Zero
...
#### System Migration
Moving Project Zero to a new server or computer:
1. Create a complete backup on the original system
2. Install Project Zero on the new system
...
#### Team Collaboration
Sharing Project Zero configurations:
...
> [!TIP]
> The backup system is designed to work across different operating systems and Project Zero installations. Your backups from a Windows system will work on Linux, and vice versa.
