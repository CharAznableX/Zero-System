...
This guide provides comprehensive technical details for configuring and utilizing external tool providers through the Model Context Protocol (MCP) with Project Zero. This allows Project Zero to leverage tools hosted by separate local or remote MCP-compliant servers.

> [!NOTE]
> For a quick start guide on adding MCP servers through the UI, see [MCP Setup](../guides/mcp-setup.md).

> [!NOTE]
> This guide covers Project Zero as an MCP **client**. To expose Project Zero as an MCP **server**, see [Connectivity → MCP Server](connectivity.md#mcp-server-connectivity).

## MCP Server Types

Project Zero supports three main types of MCP servers:

1.  **Local Stdio Servers**: Local executables that Project Zero communicates with via standard input/output (stdio).
...
## How Project Zero Consumes MCP Tools

Project Zero discovers and integrates MCP tools dynamically through the following process:

1.  **Configuration**: MCP servers are defined in the Project Zero configuration, primarily through the Settings UI.
2.  **Saving Settings**: When saved via the UI, Project Zero updates `usr/settings.json`, specifically the `"mcp_servers"` key.
3.  **Server Startup**: Project Zero initializes configured MCP servers (stdio) or connects to them (remote). For `npx`/`uvx` based servers, the first run downloads packages.
4.  **Tool Discovery**: Upon initialization, Project Zero connects to each enabled MCP server and queries for available tools, descriptions, and parameters.
...
6.  **Tool Invocation**: When the LLM requests an MCP tool, Project Zero's `process_tools` method (`mcp_handler.py`) routes the request to the appropriate MCP server.
...
### Project Zero in Docker, MCP Server on Host
...
- Final tool name in Project Zero: `sequential_thinking.run_chain`
...
- Project Zero maintains persistent connections to remote servers
...
Project Zero can chain these tools automatically based on task requirements.
...
- Test with Project Zero before production
...
- [Connectivity: MCP Server](connectivity.md#mcp-server-connectivity) - Exposing Project Zero as MCP server
