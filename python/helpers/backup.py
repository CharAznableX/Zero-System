...
class BackupService:
    """
    Core backup and restore service for Project Zero.
    ...
    def __init__(self):
        self.project_zero_version = self._get_project_zero_version()
        self.project_zero_root = files.get_abs_path("")  # Resolved Project Zero root

        # Build base paths map for pattern resolution
        self.base_paths = {
            self.project_zero_root: self.project_zero_root,
        }
    ...
    def get_default_backup_metadata(self) -> Dict[str, Any]:
        ...
        return {
            "backup_name": f"project-zero-backup-{timestamp[:10]}",
            "include_hidden": True,
    ...
    def _get_default_patterns(self) -> str:
        ...
        Only includes Project Zero project directory patterns.
        """
        # Ensure paths don't have double slashes
        agent_root = self.project_zero_root.rstrip('/')
...
"""
    ...
    def _get_project_zero_version(self) -> str:
        """Get current Project Zero version"""
        try:
    ...
    async def _get_environment_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "project_zero_root": files.get_abs_path(""),
    ...
    def _translate_patterns(self, patterns: List[str], backup_metadata: Dict[str, Any]) -> List[str]:
        ...
        Replaces the backed up Project Zero root path with the current Project Zero root path
        ...
        Args:
            ...
            backup_metadata: Backup metadata containing the original project_zero_root

        Returns:
            List of translated patterns for the current system
        """
        # Get the backed up Project Zero root path from metadata
        environment_info = backup_metadata.get("environment_info", {})
        backed_up_agent_root = environment_info.get("project_zero_root", "")

        # Get current Project Zero root path
        current_agent_root = self.project_zero_root
        ...
        for pattern in patterns:
            # Check if the pattern starts with the backed up Project Zero root
    ...
    async def create_backup(
        ...
        backup_name: str = "project-zero-backup"
    ) -> str:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                metadata = {
                    ...
                    "project_zero_version": self.project_zero_version,
                    "timestamp": datetime.datetime.now().isoformat(),
    ...
    def _translate_restore_path(self, archive_path: str, backup_metadata: Dict[str, Any]) -> str:
        ...
        # Check if the archive path starts with the backed up Project Zero root
